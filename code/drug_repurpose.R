...
#take the top 20 hits and the bottom 6 hits
...
pdf(paste(disease, "/", "cmap_predictions.pdf", sep=""))
  ...
  text(x = seq(0,1,length.out=ncol( drug_dz_signature ) ), c(-0.04),
     labels = c( disease,drug_names), srt = 45, pos=2, offset=0.01, xpd = TRUE, cex=1.2, col = c("red", cols))
dev.off()
